...
const api = {
  ...
  async fetchWithAuth(endpoint, options = {}) {
    console.log(`[Virtual API] Intercepted request to: ${endpoint}`); if(!endpoint) throw new Error("API Error: No endpoint provided");
    ...
    if (endpoint.startsWith('/saved-trips')) {
      ...
      // POST
      if (method === 'POST') {
        const tripId = body.tripId?._id || body.tripId; // Handle both object and string ID
        const existing = savedTrips.find(st => (st.tripId?._id || st.tripId) === tripId);
